...
/**
 * This class provides a set of test cases for the duplicate value behavior
 * enum.
 */
public final class DuplicateValueBehaviorTestCase
       extends LDAPSDKTestCase
{
  ...
  public void testBasic()
         throws Exception
  {
    assertNotNull(DuplicateValueBehavior.values());
    for (final DuplicateValueBehavior b : DuplicateValueBehavior.values())
    {
      assertNotNull(b);
      assertNotNull(b.name());
      assertEquals(DuplicateValueBehavior.valueOf(b.name()), b);
    }
  }
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final DuplicateValueBehavior value : DuplicateValueBehavior.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(DuplicateValueBehavior.forName(name));
        assertEquals(DuplicateValueBehavior.forName(name), value);
      }
    }

    assertNull(DuplicateValueBehavior.forName("some undefined name"));
  }
