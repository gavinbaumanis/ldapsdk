/*
 * Copyright 2009-2026 Ping Identity Corporation
 * All Rights Reserved.
 */
/*
 * Copyright 2009-2026 Ping Identity Corporation
 ...
 */
/*
 * Copyright (C) 2009-2026 Ping Identity Corporation
 *
 ...
 * {@code AddResultAccessLogMessage} class.
 */
public class AddResultAccessLogMessageTestCase
       extends LDAPSDKTestCase
{
         ...
         throws Exception
  {
    ...
    AddResultAccessLogMessage m = new AddResultAccessLogMessage(s);
    m = new AddResultAccessLogMessage(m);
    ...
    assertNull(m.getAttributeNames());
    ...
    assertNull(m.getUndeleteFromDN());
    ...
    assertNotNull(m.getIndexesWithKeysAccessedNearEntryLimit());
    assertTrue(m.getIndexesWithKeysAccessedNearEntryLimit().isEmpty());

    assertNotNull(m.getIndexesWithKeysAccessedOverEntryLimit());
    assertTrue(m.getIndexesWithKeysAccessedOverEntryLimit().isEmpty());
         ...
         throws Exception
  {
    ...
    SimpleDateFormat f = new SimpleDateFormat("'['dd/MMM/yyyy:HH:mm:ss Z']'");
    String s = f.format(d) + " ADD RESULT conn=1 op=2 msgID=3 " +
         ...
         throws Exception
  {
    ...
    String s = f.format(d) + " ADD RESULT " +
               ...
               "dn=\"ou=People,dc=example,dc=com\" attrs=\"objectClass,ou\" " +
               ...
               "undeleteFromDN=\"entryUUID=12345+ou=People,dc=example," +
                    "dc=com\" " +
               ...
               "indexesWithKeysAccessedNearEntryLimit=\"i1,i2\" " +
               "indexesWithKeysAccessedExceedingEntryLimit=\"i3,i4\" " +
    ...
    assertNotNull(m.getAttributeNames());
    assertEquals(m.getAttributeNames().size(), 2);
    assertTrue(m.getAttributeNames().contains("objectClass"));
    assertTrue(m.getAttributeNames().contains("ou"));
    ...
    assertNotNull(m.getUndeleteFromDN());
    ...
    assertNotNull(m.getIndexesWithKeysAccessedNearEntryLimit());
    assertEquals(m.getIndexesWithKeysAccessedNearEntryLimit().size(), 2);
    assertTrue(m.getIndexesWithKeysAccessedNearEntryLimit().contains("i1"));
    assertTrue(m.getIndexesWithKeysAccessedNearEntryLimit().contains("i2"));

    assertNotNull(m.getIndexesWithKeysAccessedOverEntryLimit());
    assertEquals(m.getIndexesWithKeysAccessedOverEntryLimit().size(), 2);
    assertTrue(m.getIndexesWithKeysAccessedOverEntryLimit().contains("i3"));
    assertTrue(m.getIndexesWithKeysAccessedOverEntryLimit().contains("i4"));
