       ...
       extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final SuppressType value : SuppressType.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(SuppressType.forName(name));
        assertEquals(SuppressType.forName(name), value);
      }
    }

    assertNull(SuppressType.forName("some undefined name"));
  }
