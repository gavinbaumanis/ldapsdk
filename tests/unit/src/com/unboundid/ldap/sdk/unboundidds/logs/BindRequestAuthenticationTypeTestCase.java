     ...
     extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final BindRequestAuthenticationType value :
         BindRequestAuthenticationType.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(BindRequestAuthenticationType.forName(name));
        assertEquals(BindRequestAuthenticationType.forName(name), value);
      }
    }

    assertNull(BindRequestAuthenticationType.forName("some undefined name"));
  }
