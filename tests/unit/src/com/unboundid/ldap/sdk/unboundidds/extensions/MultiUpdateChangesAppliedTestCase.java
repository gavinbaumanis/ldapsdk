       ...
       extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final MultiUpdateChangesApplied value :
         MultiUpdateChangesApplied.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(MultiUpdateChangesApplied.forName(name));
        assertEquals(MultiUpdateChangesApplied.forName(name), value);
      }
    }

    assertNull(MultiUpdateChangesApplied.forName("some undefined name"));
  }
