...
/**
 * This class provides test coverage for the intercepted in-memory modify
 * operation.
 */
public final class InterceptedModifyOperationTestCase
       extends LDAPSDKTestCase
{
         ...
         throws Exception
  {
    ...
    final ModifyRequestProtocolOp requestOp =
         new ModifyRequestProtocolOp(new ModifyRequest(
              "dn: dc=example,dc=com",
              "changetype: modify",
              "replace: description",
              "description: foo"));

    final InterceptedModifyOperation o = new InterceptedModifyOperation(
         null, 1, requestOp);
    ...
    assertEquals(
         o.getRequest().getModifications().get(0).getAttribute().getValue(),
         "foo");
    assertNotNull(o.toString());

    final ModifyRequest r = o.getRequest().duplicate();
    r.setModifications(new Modification(ModificationType.REPLACE, "description",
         "bar"));
    o.setRequest(r);
