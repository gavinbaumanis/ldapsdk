...
/**
 * This class provides a set of test cases for the {@code ErrorLogSeverity}
 * class.
 */
public class ErrorLogSeverityTestCase
     extends LDAPSDKTestCase
{
  ...
  public void testValuesAndValueOf()
         throws Exception
  {
    for (ErrorLogSeverity t : ErrorLogSeverity.values())
    {
      assertNotNull(ErrorLogSeverity.valueOf(t.name()));
      assertEquals(ErrorLogSeverity.valueOf(t.name()), t);
    }
  }
  ...
  public void testValueOfInvalid()
         throws Exception
  {
    ErrorLogSeverity.valueOf("invalid");
  }
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final ErrorLogSeverity value : ErrorLogSeverity.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(ErrorLogSeverity.forName(name));
        assertEquals(ErrorLogSeverity.forName(name), value);
      }
    }

    assertNull(ErrorLogSeverity.forName("some undefined name"));
  }
