       ...
       extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final UniquenessValidationLevel value :
         UniquenessValidationLevel.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(UniquenessValidationLevel.forName(name));
        assertEquals(UniquenessValidationLevel.forName(name), value);
      }
    }

    assertNull(UniquenessValidationLevel.forName("some undefined name"));
  }
