       ...
       extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final UniquenessMultipleAttributeBehavior value :
         UniquenessMultipleAttributeBehavior.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(UniquenessMultipleAttributeBehavior.forName(name));
        assertEquals(UniquenessMultipleAttributeBehavior.forName(name), value);
      }
    }

    assertNull(UniquenessMultipleAttributeBehavior.forName(
         "some undefined name"));
  }
