       ...
       extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final MultiUpdateErrorBehavior value :
         MultiUpdateErrorBehavior.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(MultiUpdateErrorBehavior.forName(name));
        assertEquals(MultiUpdateErrorBehavior.forName(name), value);
      }
    }

    assertNull(MultiUpdateErrorBehavior.forName("some undefined name"));
  }
