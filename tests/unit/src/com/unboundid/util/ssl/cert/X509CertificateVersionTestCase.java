 ...
 */
/*
 * Copyright (C) 2017-2026 Ping Identity Corporation
 *
...
/**
 * This class provides test coverage for the X509CertificateVersion class.
 */
public class X509CertificateVersionTestCase
       extends LDAPSDKTestCase
{
  ...
  public void testValues()
         throws Exception
  {
    for (final X509CertificateVersion v : X509CertificateVersion.values())
    {
      assertNotNull(v);
      ...
      assertNotNull(X509CertificateVersion.valueOf(v.getIntValue()));
      assertEquals(X509CertificateVersion.valueOf(v.getIntValue()), v);

      assertNotNull(X509CertificateVersion.valueOf(v.name()));
      assertEquals(X509CertificateVersion.valueOf(v.name()), v);
    }
  }
  ...
  public void testNonexistentValue()
         throws Exception
  {
    assertNull(X509CertificateVersion.valueOf(1234));

    try
    {
      X509CertificateVersion.valueOf("nonexistent");
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final X509CertificateVersion value : X509CertificateVersion.values())
    {
      for (final String name : getNames(value.name(), value.getName()))
      {
        assertNotNull(X509CertificateVersion.forName(name));
        assertEquals(X509CertificateVersion.forName(name), value);
      }
    }

    assertNull(X509CertificateVersion.forName("some undefined name"));
  }
