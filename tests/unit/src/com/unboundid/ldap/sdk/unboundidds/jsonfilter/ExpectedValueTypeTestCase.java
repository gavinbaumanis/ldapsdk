       ...
       extends LDAPSDKTestCase
{
  ...
  public void testForNameAutomated()
         throws Exception
  {
    for (final ExpectedValueType value : ExpectedValueType.values())
    {
      for (final String name : getNames(value.name()))
      {
        assertNotNull(ExpectedValueType.forName(name));
        assertEquals(ExpectedValueType.forName(name), value);
      }
    }

    assertNull(ExpectedValueType.forName("some undefined name"));
  }
