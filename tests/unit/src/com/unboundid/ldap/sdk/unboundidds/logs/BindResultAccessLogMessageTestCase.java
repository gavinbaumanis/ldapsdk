 ...
 * {@code BindResultAccessLogMessage} class.
 */
public class BindResultAccessLogMessageTestCase
       extends LDAPSDKTestCase
{
         ...
         throws Exception
  {
    ...
    BindResultAccessLogMessage m = new BindResultAccessLogMessage(s);
    m = new BindResultAccessLogMessage(m);
    ...
    assertNull(m.getAuthenticationDN());

    assertNull(m.getAuthorizationDN());

    assertNull(m.getAuthenticationFailureID());

    assertNull(m.getAuthenticationFailureReason());

    assertNull(m.getRetiredPasswordUsed());
         ...
         throws Exception
  {
    ...
    SimpleDateFormat f = new SimpleDateFormat("'['dd/MMM/yyyy:HH:mm:ss Z']'");
    String s = f.format(d) + " BIND RESULT conn=1 op=2 msgID=3 " +
         ...
         throws Exception
  {
    ...
    assertNotNull(m.getRetiredPasswordUsed());
    assertEquals(m.getRetiredPasswordUsed(), Boolean.TRUE);
