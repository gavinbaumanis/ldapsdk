/*
 * Copyright 2014-2026 Ping Identity Corporation
 * All Rights Reserved.
 */
/*
 * Copyright 2014-2026 Ping Identity Corporation
 ...
 */
/*
 * Copyright (C) 2014-2026 Ping Identity Corporation
 *
...
/**
 * This class provides test coverage for the NumericGaugeMonitorEntry class.
 */
public class NumericGaugeMonitorEntryTestCase
       extends LDAPSDKTestCase
{
         ...
         throws Exception
  {
    final Entry e = new Entry(
         ...
         "value: 12.34",
         "previous-value: 56.78",
         "value-minimum: 12.34",
         "value-maximum: 91.01",
         "observed-values: 12.34,56.78,91.01",
         "severity: NORMAL",
    ...
    final NumericGaugeMonitorEntry me = new NumericGaugeMonitorEntry(e);
    assertNotNull(me.toString());

    assertEquals(me.getMonitorClass(), "ds-numeric-gauge-monitor-entry");

    assertEquals(MonitorEntry.decode(e).getClass().getName(),
         NumericGaugeMonitorEntry.class.getName());
    ...
    assertNotNull(me.getCurrentValue());
    assertEquals(me.getCurrentValue(), Double.valueOf("12.34"));

    assertNotNull(me.getPreviousValue());
    assertEquals(me.getPreviousValue(), Double.valueOf("56.78"));

    assertNotNull(me.getMinimumValue());
    assertEquals(me.getMinimumValue(), Double.valueOf("12.34"));

    assertNotNull(me.getMaximumValue());
    assertEquals(me.getMaximumValue(), Double.valueOf("91.01"));

    assertNotNull(me.getObservedValues());
    assertFalse(me.getObservedValues().isEmpty());
    assertEquals(me.getObservedValues().size(), 3);
    ...
    assertNotNull(attrs.get("value"));
    assertFalse(attrs.get("value").hasMultipleValues());
    assertNotNull(attrs.get("value").getDoubleValue());

    assertNotNull(attrs.get("previous-value"));
    assertFalse(attrs.get("previous-value").hasMultipleValues());
    assertNotNull(attrs.get("previous-value").getDoubleValue());

    assertNotNull(attrs.get("value-minimum"));
    assertFalse(attrs.get("value-minimum").hasMultipleValues());
    assertNotNull(attrs.get("value-minimum").getDoubleValue());

    assertNotNull(attrs.get("value-maximum"));
    assertFalse(attrs.get("value-maximum").hasMultipleValues());
    assertNotNull(attrs.get("value-maximum").getDoubleValue());

    assertNotNull(attrs.get("observed-values"));
    assertTrue(attrs.get("observed-values").hasMultipleValues());
    assertNotNull(attrs.get("observed-values").getDoubleValues());
         ...
         throws Exception
  {
    ...
    assertNull(me.getCurrentValue());

    assertNull(me.getPreviousValue());

    assertNull(me.getMinimumValue());

    assertNull(me.getMaximumValue());

    assertNotNull(me.getObservedValues());
    assertTrue(me.getObservedValues().isEmpty());
