/*
 * Copyright 2012-2026 Ping Identity Corporation
 * All Rights Reserved.
 */
/*
 * Copyright 2012-2026 Ping Identity Corporation
 ...
 */
/*
 * Copyright (C) 2012-2026 Ping Identity Corporation
 *
 ...
 * {@code EntryRebalancingResultAccessLogMessage} class.
 */
public class EntryRebalancingResultAccessLogMessageTestCase
       extends LDAPSDKTestCase
{
         ...
         throws Exception
  {
    ...
    EntryRebalancingResultAccessLogMessage m =
         new EntryRebalancingResultAccessLogMessage(s);
    m = new EntryRebalancingResultAccessLogMessage(m);
    ...
    assertNull(m.getAdminActionRequired());

    assertNull(m.sourceAltered());

    assertNull(m.targetAltered());

    assertNull(m.getEntriesReadFromSource());

    assertNull(m.getEntriesAddedToTarget());

    assertNull(m.getEntriesDeletedFromSource());
         ...
         throws Exception
  {
    ...
    SimpleDateFormat f = new SimpleDateFormat("'['dd/MMM/yyyy:HH:mm:ss Z']'");
    String s = f.format(d) + " ENTRY-REBALANCING-RESULT rebalancingOp=1 " +
         "triggeredByConn=2 triggeredByOp=3 resultCode=80 " +
