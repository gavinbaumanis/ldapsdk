 ...
 */
/*
 * Copyright (C) 2010-2026 Ping Identity Corporation
 *
...
/**
 * This class provides test coverage for the RefreshCertificateMonitorTask
 * class.
 */
public class RefreshCertificateMonitorTaskTestCase
       extends LDAPSDKTestCase
{
  /**
   * Tests the basic behavior for the task.
   *
  ...
  public void testBasicBehavior()
         throws Exception
  {
    final RefreshCertificateMonitorTask t = new RefreshCertificateMonitorTask();
  ...
  public void testConstructor4Valid(final Entry e)
         throws Exception
  {
    final RefreshCertificateMonitorTask t =
         new RefreshCertificateMonitorTask(e);
    ...
    assertTrue(Task.decodeTask(e) instanceof RefreshCertificateMonitorTask);
  }



  /**
   * Retrieves a set of entries that may be parsed as valid refresh certificate
   * monitor task definitions.
   *
   * @return  A set of entries that may be parsed as valid refresh certificate
   *          monitor task definitions.
   *
   * @throws  Exception  If an unexpected problem occurs.
   */
    ...
    return new Object[][]
    {
      new Object[]
      {
        new Entry("dn: ds-task-id=validTask1,cn=Scheduled Tasks,cn=tasks",
                  "objectClass: top",
                  "objectclass: ds-task",
                  "objectclass: ds-task-refresh-certificate-monitor",
                  "ds-task-id: validTask1",
                  "ds-task-class-name: com.unboundid.directory.server." +
                       "tasks.RefreshCertificateMonitorTask",
         ...
         throws Exception
  {
    try
    {
      assertFalse(Task.decodeTask(e) instanceof RefreshCertificateMonitorTask);
    }
   ...
   * certificate monitor task definitions.
   *
   * @return  A set of entries that cannot be parsed as valid refresh
   *          certificate monitor task definitions.
   *
   * @throws  Exception  If an unexpected problem occurs.
   */
         ...
         throws Exception
  {
    ...
    final RefreshCertificateMonitorTask t =
         new RefreshCertificateMonitorTask(properties);
